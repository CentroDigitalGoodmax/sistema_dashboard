...
try {
    ...
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] == 0) {
        ...
        $upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/assets/visados';
        ...
        $imagen_url = '/assets/visados/' . $filename;
    ...
    if ($id > 0) {
        ...
        // INSERTAR
        $query = "INSERT INTO visados (persona, img, estado) VALUES (?, ?, ?)";
    ...
    if (mysqli_stmt_execute($stmt)) {
        $response = ['success' => true, 'message' => 'Visado guardado correctamente'];
        if ($id == 0) {
