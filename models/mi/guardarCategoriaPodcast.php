...
try {
    ...
    if ($id > 0) {
        $query = "UPDATE categoria_podcast SET nombre = ?, slug = ?, descripcion = ?, color = ?, orden = ?, activo = ? WHERE id = ?";
        $stmt = mysqli_prepare($conex, $query);
        mysqli_stmt_bind_param($stmt, "ssssiii", $nombre, $slug, $descripcion, $color, $orden, $activo, $id);
    } else {
        $query = "INSERT INTO categoria_podcast (nombre, slug, descripcion, color, orden, activo, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conex, $query);
        mysqli_stmt_bind_param($stmt, "ssssiis", $nombre, $slug, $descripcion, $color, $orden, $activo, $created_at);
    }
